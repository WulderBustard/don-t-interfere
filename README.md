...
`npm run setup` устанавливает зависимости и при отсутствии сертификатов создаёт self-signed сертификат для текущего LAN-IP. Для явного адреса используйте `HOST=192.168.0.0 npm run cert:dev`.
...
Для рабочего домена задайте `SSL_KEY_PATH` и `SSL_CERT_PATH` в обоих `.env`. Приватные LAN-адреса вида `192.x.x.x` не могут получить публичный сертификат Let's Encrypt; для них используйте локальный доверенный CA, например `mkcert`.
